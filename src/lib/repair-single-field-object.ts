 ...
 * broken output.
 */
export async function generateSingleFieldObject<T extends Record<string, unknown>>({ model, abortSignal, system, prompt, key, schema }: { model: LanguageModelV3; abortSignal?: AbortSignal; system: string; prompt: string; key: string; schema: z4.ZodType<T> }): Promise<T> {
	try {
...
async function repairText({ text, key, sourceText, model, abortSignal, errorMessage }: { text: string; key: string; sourceText: string; model: LanguageModelV3; abortSignal?: AbortSignal; errorMessage: string }): Promise<string | null> {
...
function tryStructuralRepair(text: string, key: string, sourceText: string): string | null {
...
function isEcho(value: string, sourceText: string): boolean {
...
function jsonrepairSafe(text: string): string {
	try {
